...
Hasil algoritmaGenetik(int N, int ukuranPopulasi, int maxGenerasi, double tingkatCrossover, double tingkatMutasi) {
    ...
    for (int gen = 0; gen < maxGenerasi; ++gen) {
        ...
        hasil.errorAkhir = 1.0 / terbaik.fitness - 1.0;
        hasil.riwayatError.push_back(hasil.errorAkhir);
        hasil.langkah = gen + 1;
        std::cout << "Generasi: " << hasil.langkah << ", Error Terbaik: " << hasil.errorAkhir << std::endl;
        if (terbaik.fitness >= 1.0)
            break;
    }
    auto waktuAkhir = std::chrono::high_resolution_clock::now();
    hasil.waktuEksekusi = std::chrono::duration<double>(waktuAkhir - waktuMulai).count();
    return hasil;
}
